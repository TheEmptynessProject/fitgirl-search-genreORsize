...
(function() {
	...
	const genresWanted = [];
	const maxSize = convertToMB('5 GB')
	const packsizeBool = true; //Change to check either repack size (true) or real size (false)
	const posts = document.getElementsByClassName("post");
	let stopScript = false; // flag to track whether the button has been clicked
	for (const post of posts) {
		if (stopScript) {
			break; // stop script execution if button has been clicked
		}
